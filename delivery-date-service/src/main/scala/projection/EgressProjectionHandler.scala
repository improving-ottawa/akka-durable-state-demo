...
class EgressProjectionHandler(
  ...
  override def process(
    envelope: DurableStateChange[DeliveryDateEntity.DeliveryDateState]
  ): Future[Done] =
    envelope match {
      case update: UpdatedDurableState[_] =>
        val event = toEgressEvent(update.value)
        val producerRecord =
          new ProducerRecord(topic, event.id.toString, event.asJson.noSpaces)
